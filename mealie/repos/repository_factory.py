...
from mealie.db.models.recipe.tag import (
    Tag,
    cookbooks_to_tags,
    plan_rules_to_tags,
    recipes_to_tags,
...
class RepositoryCategories(GroupRepositoryGeneric[CategoryOut, Category]):
    ...
    def merge(self, from_category: UUID4, to_category: UUID4) -> CategoryOut | None:
        already_in_recipes = select(recipes_to_categories.c.recipe_id).where(
            recipes_to_categories.c.category_id == to_category
        )
        already_in_cookbooks = select(cookbooks_to_categories.c.cookbook_id).where(
            cookbooks_to_categories.c.category_id == to_category
        )
        already_in_plan_rules = select(plan_rules_to_categories.c.group_plan_rule_id).where(
        ...
        try:
            with self.session.begin_nested():
                ...
                self.session.execute(
                    ...
                    .where(recipes_to_categories.c.recipe_id.not_in(already_in_recipes))
                ...
                self.session.execute(
                    ...
                    .where(cookbooks_to_categories.c.cookbook_id.not_in(already_in_cookbooks))
                    .values(category_id=to_category)
                )
                self.session.execute(
                ...
                self.session.execute(
                    ...
                    .where(plan_rules_to_categories.c.group_plan_rule_id.not_in(already_in_plan_rules))
                    .values(category_id=to_category)
                )
                self.session.execute(
...
class RepositoryTags(GroupRepositoryGeneric[TagOut, Tag]):
    ...
    def merge(self, from_tag: UUID4, to_tag: UUID4) -> TagOut | None:
        already_in_recipes = select(recipes_to_tags.c.recipe_id).where(recipes_to_tags.c.tag_id == to_tag)
        already_in_cookbooks = select(cookbooks_to_tags.c.cookbook_id).where(cookbooks_to_tags.c.tag_id == to_tag)
        already_in_plan_rules = select(plan_rules_to_tags.c.plan_rule_id).where(plan_rules_to_tags.c.tag_id == to_tag)
        ...
        try:
            with self.session.begin_nested():
                ...
                self.session.execute(
                    ...
                    .where(recipes_to_tags.c.recipe_id.not_in(already_in_recipes))
                ...
                self.session.execute(
                    ...
                    .where(cookbooks_to_tags.c.cookbook_id.not_in(already_in_cookbooks))
                    .values(tag_id=to_tag)
                )
                ...
                self.session.execute(
                    ...
                    .where(plan_rules_to_tags.c.plan_rule_id.not_in(already_in_plan_rules))
                    .values(tag_id=to_tag)
                )
